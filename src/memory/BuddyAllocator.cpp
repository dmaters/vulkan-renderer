...
BuddyAllocator::BuddyAllocator(
	vk::MemoryPropertyFlags memory, uint32_t size, uint32_t minimumAlignment
) :
	...
	m_minAlignment =
		std::max(minimumAlignment, (uint32_t)(1 << std::max(m_levels - 20, 1)));
...
std::optional<SubAllocation> BuddyAllocator::subAllocate(
	...
	assert(
		offset % m_minAlignment == 0 &&
		(requirements.alignment == 0 || offset % requirements.alignment == 0)
	);
