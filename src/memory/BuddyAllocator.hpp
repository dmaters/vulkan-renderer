...
private:
	...
	uint32_t getSizeFromLevel(uint8_t level) { return 1 << (level + m_baseLevel); }
	uint8_t getLevel(uint32_t size) { return std::max(m_baseLevel, (uint8_t)std::log2(size)) - m_baseLevel; }

public:
	BuddyAllocator(vk::MemoryPropertyFlags, uint32_t size, uint32_t minAlignment);
