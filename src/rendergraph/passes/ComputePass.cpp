...
void ComputePass(Task::BuildContext& context, MaterialIndex materialIndex, glm::uvec3 dispatchSize) {
	...
	context.commandBuffer.bindPipeline(vk::PipelineBindPoint::eCompute, material.pipeline);

	vk::DescriptorSet textureSet = context.materialManager.getTextureSet();

	context.commandBuffer.bindDescriptorSets(
		vk::PipelineBindPoint::eCompute, material.pipelineLayout, 1, { textureSet }, {}
	);
	...
	if (descriptors.descriptors.size() > 0) {
		context.commandBuffer.pushDescriptorSetKHR(
			vk::PipelineBindPoint::eCompute, material.pipelineLayout, 0, descriptors.descriptors
		);
	} else
		context.commandBuffer.bindDescriptorSets(
